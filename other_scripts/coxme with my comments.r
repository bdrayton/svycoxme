...
function (formula, data, weights, subset, na.action, init, control,
    ties = c("efron", "breslow"), varlist, vfixed,
    vinit, x = FALSE, y = TRUE, refine.n = 0, random, fixed,
    variance, ...)
{

# Checking aguments passed to the function
    ties <- match.arg(ties)
    Call <- match.call()
    if (!missing(fixed)) {
        if (missing(formula)) {
    ...
    if (!missing(random)) {
        ...
        if (class(random) != "formula" || length(random) !=
            2)
            stop("Invalid random formula")
        j <- length(formula)
        formula[[j]] <- call("+", formula[[j]], call("(",
            random[[2]]))
    }
    if (!missing(variance)) {
...
#
# assign data used in the model to "m"
    temp <- call("model.frame", formula = subbar(formula))
    for (i in c("data", "subset", "weights",
        "na.action")) if (!is.null(Call[[i]]))
        temp[[i]] <- Call[[i]]
    if (is.R())
        m <- eval.parent(temp)
...
# Checks on the response variable
    ...
    n <- nrow(Y)
    if (n == 0)
        stop("No observations remain in the data set")
    if (!inherits(Y, "Surv"))
    ...
    if (type != "right" && type != "counting")
        stop(paste("Cox model doesn't support '", type,
            "' survival data", sep = ""))
#
# Checks on the weights variable, if there is one
...
#
# Checks on the offsets if there are any
    offset <- model.offset(m)
    if (length(offset) == 0)
        offset <- rep(0, n)
    pterms <- sapply(m, inherits, "coxph.penalty")
# disallow penalty terms.
    if (any(pterms)) {
...
# set control parameters
    ...
    if (missing(init))
        init <- NULL
# check the formula
# split model formula into list of fixed and random terms
    flist <- formula1(formula)
    if (hasAbar(flist$fixed))
        stop("Invalid formula: a '|' outside of a valid random effects term")
# add information about clusters, strata. clusters are disallowed.
...
# X matrix of fixed effects is made. if strata are present, they are not included
# in X, but held is "strats"
    if (length(strats)) {
...
# check the random effects. must be >0
    ...
    if (nrandom == 0)
        stop("No random effects terms found")
...
# set up variance estimation for random effect variance parameters
    ...
    if (!is.null(names(varlist))) {
        ...
        if (any(indx == 0 & vname != ""))
            stop(paste("Varlist element not matched:",
                vname[indx == 0 & vname != ""]))
        if (any(indx > 0)) {
...
# end of variance set up.
#
# define a bunch of functions in the middle of this function.
    getcmat <- function(x, mf) {
        ...
        if (any(ftemp)) {
            clist <- lapply(mf[varnames[ftemp]], function(x) diag(length(levels(x))))
    ...
    getgroups <- function(x, mf) {
        ...
        if (any(indx == 0))
            stop(paste("Invalid grouping factor", varname[indx ==
                0]))
...
#
    if (missing(vinit) || is.null(vinit))
        vinit <- vector("list", nrandom)
    else {
        if (nrandom == 1) {
            if (is.numeric(vinit))
                vinit <- list(vinit)
            else if (is.list(vinit))
                vinit <- list(unlist(vinit))
        }
        if (!is.list(vinit))
            stop("Invalid value for `vinit` parameter")
        if (length(vinit) > nrandom)
            stop(paste("Vinit must be a list of length",
                nrandom))
        if (!all(sapply(vinit, function(x) (is.null(x) || is.numeric(x)))))
            stop("Vinit must contain numeric values")
        if (length(vinit) < nrandom)
            vinit <- c(vinit, vector("list", nrandom -
                length(vinit)))
        tname <- names(vinit)
        if (!is.null(tname)) {
    ...
    if (missing(vfixed) || is.null(vfixed))
        vfixed <- vector("list", nrandom)
    else {
        if (nrandom == 1) {
            if (is.numeric(vfixed))
                vfixed <- list(vfixed)
            else if (is.list(vfixed))
                vfixed <- list(unlist(vfixed))
        }
        if (!is.list(vfixed))
            stop("Invalid value for `vfixed` parameter")
        if (length(vfixed) > nrandom)
            stop(paste("Vfixed must be a list of length",
                nrandom))
        if (!all(sapply(vfixed, function(x) (is.null(x) || is.numeric(x)))))
            stop("Vfixed must contain numeric values")
        if (length(vfixed) < nrandom)
            vfixed <- c(vfixed, vector("list", nrandom -
                length(vfixed)))
        tname <- names(vfixed)
        if (!is.null(tname)) {
    ...
    newzmat <- function(xmat, xmap) {
        ...
        for (i in 1:ncol(xmap)) newz[cbind(1:n, xmap[, i])] <- xmat[,
            i]
        newz
    }
    fmat <- zmat <- matrix(0, nrow = n, ncol = 0)
    ntheta <- integer(nrandom)
    ncoef <- matrix(0L, nrandom, 2, dimnames = list(NULL, c("intercept",
        "slope")))
    itheta <- NULL
    for (i in 1:nrandom) {
        f2 <- formula2(flist$random[[i]])
        if (f2$intercept & f2$group == 1)
            stop(paste("Error in random term ", i, ": Random intercepts require a grouping variable",
                sep = ""))
        vfun <- varlist[[i]]
        if (!is.null(f2$interaction))
            stop("Interactions not yet written")
        ...
        if (!is.null(ifun$error))
            stop(paste("In random term ", i, ": ",
                ifun$error, sep = ""))
        vparm[[i]] <- ifun$parms
        if (!is.null(ifun$is.variance))
            is.variance[i] <- ifun$is.variance
        itheta <- c(itheta, ifun$theta)
        ntheta[i] <- length(ifun$theta)
        if (f2$intercept) {
            if (!is.matrix(ifun$imap) || nrow(ifun$imap) != n)
                stop(paste("In random term ", i, ": Invalid intercept matrix F",
                  sep = ""))
            temp <- sort(unique(c(ifun$imap)))
            if (any(temp != 1:length(temp)))
                stop(paste("In random term ", i, ": intercept matrix has an invalid element",
                  sep = ""))
            if (ncol(fmat) > 0)
                fmat <- cbind(fmat, ifun$imap + max(fmat))
            else fmat <- ifun$imap
            ncoef[i, 1] <- 1 + max(ifun$imap) - min(ifun$imap)
        }
        if (length(cmat) > 0) {
            if (is.null(ifun$xmap) || is.null(ifun$X) || !is.matrix(ifun$xmap) ||
                !is.matrix(ifun$X) || nrow(ifun$xmap) != n ||
                nrow(ifun$X) != n || ncol(ifun$xmap) != ncol(ifun$X))
                stop(paste("In random term ", i, "invalid X/xmap pair"))
            if (f2$intercept)
                xmap <- ifun$xmap - max(ifun$imap)
            else xmap <- ifun$xmap
            if (any(sort(unique(c(xmap))) != 1:max(xmap)))
                stop(paste("In random term ", i, ": xmap matrix has an invalid element",
                  sep = ""))
...
# the model is fitted using all the set up to this point.
    fit <- coxme.fit(X, Y, strats, offset, init, control, weights = weights,
        ties = ties, row.names(m), fmat, zmat, varlist, vparm,
        itheta, ntheta, ncoef, refine.n, is.variance = any(is.variance))
# checks on various components of the returned fit. Some post-fit processing.
    if (is.character(fit)) {
        fit <- list(fail = fit)
        oldClass(fit) <- "coxme"
        return(fit)
    ...
    if (length(zmat)) {
        indx <- if (length(fmat) > 0)
            max(fmat)
        else 0
        for (i in 1:ncol(zmat)) rlinear <- rlinear + fit$frail[indx +
            i] * zmat[, i]
    }
    if (nvar == 0)
        fit$linear.predictor <- rlinear
    else fit$linear.predictor <- as.vector(rlinear + c(X %*%
        fit$coef))
    newtheta <- random.coef <- list()
    nrandom <- length(varlist)
    sindex <- rep(1:nrandom, ntheta)
    bindex <- rep(row(ncoef), ncoef)
    for (i in 1:nrandom) {
        temp <- varlist[[i]]$wrapup(fit$theta[sindex == i], fit$frail[bindex ==
            i], vparm[[i]])
    ...
    fit$call <- Call
    fit$ties <- ties
    names(fit$loglik) <- c("NULL", "Integrated",
        "Penalized")
    oldClass(fit) <- "coxme"
    fit
}
